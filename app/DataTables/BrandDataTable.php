    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $editBtn = "<a href='" . route('admin.brand.edit', $query->id) . "' class='btn btn-primary btn-sm'><i class='far fa-edit'></i></a>";
                $deleteBtn = "<a href='" . route('admin.brand.destroy', $query->id) . "' class='btn btn-danger btn-sm ml-2 delete-item'><i class='far fa-trash-alt'></i></a>";

                return $editBtn . $deleteBtn;
            })
            ->addColumn('logo', function ($query) {
                return $img = "<img src='" . asset($query->logo) . "' style='width: 100px; height: 100px;'>";
            })
            ->addColumn('is_featured', function ($query) {
                $active = "<i class='badge badge-success'>Yes</i>";
                $inactive = "<i class='badge badge-danger'>No</i>";

                return $query->is_featured ? $active : $inactive;
            })
